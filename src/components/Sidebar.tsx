...
function NavLink({to, icon, children}: NavLinkProps) {
    const baseClassName = "flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors text-[#666] hover:bg-[#f8f8f6]"
    const activeClassName = "flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors bg-[#f8f8f6] text-[#333] font-medium"
    ...
    return (
        <div className="p-2 mb-6">
            <div className="flex items-center gap-2">
                <div
                    className="h-8 w-8 rounded-md bg-[#f8f3e7] flex items-center justify-center text-[#c28b3b] font-semibold">
                    Q
                </div>
                <div>
    ...
    return (
        <div className="mt-auto pt-4 border-t border-[#eaeaea]">
            <div className="flex items-center justify-between p-2">
                <div className="flex items-center gap-2">
          <span
              className="relative flex shrink-0 overflow-hidden rounded-full h-8 w-8 bg-[#f0f0f0] items-center justify-center text-[#666] font-medium">
            JS
          </span>
                    <div>
                        <div className="text-sm font-medium text-[#333]">Jane Smith</div>
                        <div className="text-xs text-[#888]">jane@example.com</div>
                    </div>
                </div>
                <button type="button" className="text-[#888] hover:text-[#333]">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
                         stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
                         aria-hidden="true" aria-labelledby="userMenuIcon">
                        <title id="userMenuIcon">User menu</title>
...
function NavSection({title, items}: NavSectionProps) {
    return (
        <div className={title ? "mt-8" : ""}>
            {title && <div className="text-xs font-medium text-[#888] px-3 mb-2">{title}</div>}
